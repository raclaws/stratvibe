...
/**
 * stratvibe init - KISS implementation
 * One command. One job. No questions asked.
 * 
 * Creates substrate v0.1 structure with static templates.
 * No prompts, no config, no detection logic.
 * Just like git init.
 */
...
function parseArgs() {
  ...
  // Simple flag parsing (KISS)
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`stratvibe init - KISS substrate initialization
    
Usage:
  stratvibe init           # Initialize substrate v0.1 in current directory
  stratvibe init --help    # Show this help
...
Created by R & GIaL - 2026`);
  ...
  if (args.includes('--version') || args.includes('-v')) {
    console.log('stratvibe init v0.1.0 (substrate v0.1)');
    process.exit(0);
  }
  
  // For now, ignore other flags (KISS)
  // Future: --force to overwrite, --version 0.1 to pin substrate version
}
